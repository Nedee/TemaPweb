...
public class CategoriesController : AuthorizedController
{
    ...
    /// </summary>
    public CategoriesController(IUserService userService, ICategoriesService categoriesService) : base(userService)
    {
    ...
    public async Task<ActionResult<RequestResponse>> AddCategory([FromForm] CategoryAddDTO form) // The FromForm attribute will bind each field from the form request to the properties of the UserFileAddDTO parameter.
                                                                                                // For files the property should be IFormFile or IFormFileCollection.
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ?
            this.FromServiceResponse(await CategoriesService.AddCategory(form, currentUser.Result)) :
